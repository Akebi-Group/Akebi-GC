...
namespace cheat::feature
{
	...
	InteractiveMap::InteractiveMap() : Feature(),
		NFEX(f_Enabled, u8"地图", "m_InteractiveMap", "InteractiveMap", false, false),
		NF(f_SeparatedWindows, u8"分隔窗", "InteractiveMap", true),
		...
		NF(f_ShowCompleted, u8"显示完成", "InteractiveMap", false),
		NF(f_CompletePointTransparency, u8"完成点透明度", "InteractiveMap", 0.5f),
		NF(f_ShowInCompleted, u8"完成点透明度", "InteractiveMap", true),
		NF(f_InCompletePointTransparency, u8"未完成的点透明度", "InteractiveMap", 1.0f),

		NF(f_AutoDetectNewItems, u8"检测新项目", "InteractiveMap", true),
		NF(f_AutoFixItemPositions, u8"固定物品位置", "InteractiveMap", true),
		NF(f_ObjectCheckOnlyShowed, u8"检测只显示", "InteractMap", true),
		NF(f_ObjectDetectRange, u8"检测范围", "InteractiveMap", 20.0f),
		NF(f_CheckObjectsDelay, u8"检测延迟 (ms)", "InteractiveMap", 2000),

		NF(f_AutoDetectGatheredItems, u8"检测收集到的物品", "InteractiveMap", true),
		NF(f_GatheredItemsDetectRange, u8"检测范围", "InteractiveMap", 20.0f),

		NF(f_CompleteNearestPoint, u8"完成最近点", "InteractiveMap", Hotkey()),
		NF(f_RevertLatestCompletion, u8"恢复最新完成", "InteractiveMap", Hotkey()),
		NF(f_CompleteOnlyViewed, u8"完整只显示", "InteractiveMap", true),
		NF(f_PointFindRange, u8"点查找范围", "InteractiveMap", 30.0f),

		NFS(f_CustomPointIndex, u8"自定义点索引", "InteractiveMap", 1000000),
		NFS(f_LastUserID, u8"最后一个用户ID", "InteractiveMap", 0),
	...
	{
		static const FeatureGUIInfo info{ u8"地图", u8"世界", false };
		return info;
	}
	...
	void InteractiveMap::DrawMenu()
	{
		...
		ImGui::BeginGroupPanel(u8"正在/完成的图标视图");
		{
			ConfigWidget(f_ShowCompleted, u8"显示完成点。");
			ConfigWidget(f_CompletePointTransparency, 0.01f, 0.0f, 1.0f, u8"完成点透明度。");
		...
		ImGui::BeginGroupPanel(u8"项目调整");
		{
			...
			ConfigWidget(f_ObjectCheckOnlyShowed, u8"仅针对显示的过滤器检测对象.");

			ConfigWidget(f_ObjectDetectRange, 0.1f, 5.0f, 30.0f,
				u8"固定位置：只有在该实体位置范围内找到项目时，\n\t 它的位置才会被固定.\n"
				u8"新项目检测：仅当在该实体位置范围内未找到项目时，\n\t 将其检测为新项目."
			);

			ConfigWidget(f_CheckObjectsDelay, 10, 100, 100000, u8"调整项目为耗电操作。\n"
				u8"所以重新扫描将以指定的延迟发生.");
		}
		ImGui::EndGroupPanel();

		ImGui::BeginGroupPanel(u8"采集检测");
		{
			...
			ConfigWidget(f_GatheredItemsDetectRange, 0.1f, 5.0f, 30.0f,
				u8"当实体被收集到在这个范围内找到最近的点时。");
		}
		ImGui::EndGroupPanel();

		ImGui::BeginGroupPanel(u8"手动完成");
		{
			ConfigWidget(f_CompleteNearestPoint, true, u8"按下时，完成最接近头像的点。");
			ConfigWidget(f_RevertLatestCompletion, true, u8"按下时，恢复最近的完整操作。");
			ConfigWidget(f_CompleteOnlyViewed, u8"仅对可见点执行完整。");
			ConfigWidget(f_PointFindRange, 0.5f, 0.0f, 200.0f, u8"完成在指定范围内执行。 如果 0 则为无限。");
		}
		ImGui::EndGroupPanel();
	}
	...
	void InteractiveMap::DrawFilters(const bool searchFixed)
	{
		...
		if (m_ScenesData.count(sceneID) == 0)
			ImGui::Text(u8"对不起。 不支持当前场景。");
		
		ImGui::InputText(u8"搜索", &m_SearchText); ImGui::SameLine();
		HelpMarker(
			u8"此页面后面带有项目过滤器.\n"
			u8"被激活的项目将出现在迷你/全部地图上。 （明显地）\n"
			u8"每个过滤器都有选项，您可以通过点击过滤器上的右键来访问它\n"
			u8"过滤器可以用彩色线条标记，\n"
			u8"\t它们表明过滤器支持某些功能。 （悬停）\n"
			u8"目前为止就这样了。 使用愉快 ^)"
		);
	...
	void InteractiveMap::DrawFilter(LabelData& label)
	{
		...
		if (ImGui::IsItemClicked(ImGuiMouseButton_Right))
			ImGui::OpenPopup(u8"过滤器选项");

		if (ImGui::BeginPopup(u8"过滤器选项", ImGuiWindowFlags_AlwaysAutoResize))
		{
			if (ImGui::Button(u8"掉落进度"))
			{
