...
namespace cheat::feature
{
	...
	ESP::ESP() : Feature(),
		...
		NF(f_MinSize, u8"最小实体尺寸", "ESP", 0.5f),
		NF(f_Range, u8"距离", "ESP", 100.0f),
		m_Search({})
	{
		InstallFilters();
	...
	{
		static const FeatureGUIInfo info{ "", u8"透视", false };
		return info;
	}

	void ESP::DrawMain()
	{
		if (ImGui::BeginGroupPanel(u8"全局", true))
		{
			ConfigWidget(u8"开启透视", f_Enabled, u8"无视障碍显示物品位置");
			ConfigWidget(u8"距离 (米)", f_Range, 1.0f, 1.0f, 200.0f);

			ConfigWidget(f_DrawBoxMode, u8"选择绘制模式");
			ConfigWidget(f_DrawTracerMode, u8"选择跟踪模式.");
      
			ConfigWidget(f_Fill);
			ConfigWidget(f_FillTransparency, 0.01f, 0.0f, 1.0f, u8"填充绘制方框的透明度");
			...
			if (ImGui::BeginGroupPanel(u8"全局颜色", true))
			{
				...
				ConfigWidget(f_GlobalBoxColor, u8"矩形字体颜色");
				ConfigWidget(f_GlobalLineColor, u8"线条字体颜色");
				ConfigWidget(f_GlobalRectColor, u8"追踪字体颜色");
			}
			ImGui::EndGroupPanel();
		...
		ImGui::Text(u8"如何使用过滤器:\n\t鼠标左键 - 切换该物品透视状态\n\t鼠标右键 - 打开拾色器");
		ImGui::InputText(u8"搜索过滤", &m_Search);
